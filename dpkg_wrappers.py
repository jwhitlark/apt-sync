...
def check_packages(pkgs):
    ...
    p.wait()
    Failed = [x.strip() for x in p.stdout.readlines() if x.startswith('W:')]
    bad = [x.split()[-1] for x in Failed]
    good = list(set(pkgs).difference(set(bad)))
    return (good, bad)
...
def install_mod_file(filename):
    ...
    (pkgs, bad_pkgs) = check_packages(pgks)
